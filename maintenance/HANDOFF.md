...
- 已建立專用 D1、資料表與唯一索引，部署 `quick-check-grades`，設定私人教師碼。Worker 與正式 Pages 已發布成功（功能提交 `3c496ea`）；本機練習與正式網址期中測驗交卷成功，教師查詢與 CSV 匯出確認正確。
...
- 三筆部署測試成績已備份到本機 `private/cloudflare-verification-rows.json`，並依精確作答識別碼從正式資料庫清除，避免混入學生成績。
- 正式端點驗證：未帶教師碼查詢回應 401；測驗重送只保留一筆。教師預覽與錯題重練後 D1 紀錄數不增加。
- 瀏覽器測驗的原生確認視窗曾使自動操作逾時，後續計時到期自動交卷成功；不是 Cloudflare 授權問題。已核對練習與測驗兩種模式，尚未由老師用另一台實體手機／電腦實測。
